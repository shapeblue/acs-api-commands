...
def parse_commands(xml_file, version_filters=None, old_xml_file=None):
    ...
    for cmd in root.findall('command'):
        ...
        if version_filters:
            ...
            # If no version filters, use all new parameters
            new_req_args = new_params
...
def main():
    ...
    parser.add_argument('--since', help='Comma-separated list of sinceVersion values to filter (e.g., 4.20.1,4.20.1.0). If not provided, will compare all parameters between versions.')
    parser.add_argument('--output-dir', help='Output directory (default: current directory)')
    args = parser.parse_args()

    version_filters = [v.strip() for v in args.since.split(',')] if args.since else None
